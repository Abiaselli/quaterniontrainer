...
tokenizer_path = r"/home/austin/vccode/quaterniontrainer-main"  
...
class QuaternionAttention(nn.Module):
    ...
    def forward(self, query, key, value, mask=None):
        ...
        attention_scores = attention_scores / math.sqrt(Q.size(-2) * 4+1e-6)
...
class QuaternionTransformer(nn.Module):
    ...
    def generate(self, prompt, max_new_tokens=50, seq_len=32, return_text=True):
        ...
        for _ in range(max_new_tokens):
            # Get current window
            window = generated[-seq_len:] if len(generated) > seq_len else generated
    ...
    def forward(self, src, tgt=None, mode="eval"):
        ...
        out = tgt_embed.permute(0, 1, 2,3).contiguous().view(tgt_embed.size(0), tgt_embed.size(1),tgt_embed.size(2) * tgt_embed.size(3))
        print(out.shape)
        with torch.no_grad():
            if tgt is not None:
                predicted_targets = self.embedding(tgt)
                predicted_targets = predicted_targets.permute(0, 1, 2,3).contiguous().view(predicted_targets.size(0), predicted_targets.size(1),predicted_targets.size(2) * predicted_targets.size(3))
                predicted_targets = self.fc_out(predicted_targets)
        return self.fc_out(out), predicted_targets

class QuaternionTransformer2(nn.Module):
    def __init__(self, vocab_size, embedding_dim, hidden_dim, seq_length, num_layers, num_heads, tokenizer, conversion_method='learned'):
