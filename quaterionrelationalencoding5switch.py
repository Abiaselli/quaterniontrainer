...
class QuaternionFeedForward(nn.Module):
    ...
    def forward(self, x):
        logging.debug(f"Shape of x to ff: {x.shape}")

        # x: (batch, seq_len, embedding_dim, 4)
        x = self.fc1(x)
        logging.debug(f"Shape of x after fc1: {x.shape}")

        x = self.activation(x)
        logging.debug(f"Shape of x after self.activation: {x.shape}")

        x = self.fc2(x)
        logging.debug(f"Shape of x after fc2: {x.shape}")
...
class GravitationalRelationalEncoding(nn.Module):
    """
    ...
    def __init__(self, vocab_size, embedding_dim, seq_length, zipf_factor=1.5, device="cuda" if torch.cuda.is_available() else "cpu"):
        super(GravitationalRelationalEncoding, self).__init__()
        self.vocab_size = vocab_size
        self.embedding_dim = embedding_dim
        self.seq_length = seq_length
...
class QuaternionTransformerBlockRelational(nn.Module):
    """
    Transformer block with relational learning instead of attention.
    """
    def __init__(self, embedding_dim, hidden_dim):
        super(QuaternionTransformerBlockRelational, self).__init__()
        self.feed_forward = QuaternionFeedForward(embedding_dim, hidden_dim)
        self.norm1 = QuaternionLayerNorm(embedding_dim)
        self.norm2 = QuaternionLayerNorm(embedding_dim)
...
class QuaternionTransformerRelational(nn.Module):
    """
    ...
    def __init__(self, vocab_size, embedding_dim, hidden_dim, seq_length, num_layers):
        super(QuaternionTransformerRelational, self).__init__()
        self.embedding = QuaternionEmbedding(vocab_size, embedding_dim)  # Embedding layer
        self.relational_encoding = GravitationalRelationalEncoding(vocab_size, embedding_dim, seq_length) #relational encoding
        self.relational_to_vocab = nn.Linear(embedding_dim, vocab_size)  # Learnable projection
        self.vocab_size=vocab_size
        self.layers = nn.ModuleList([
            QuaternionTransformerBlockRelational(embedding_dim, hidden_dim)
            for _ in range(num_layers)
        ])
        # Introduce the quaternion-to-real conversion module.
...
class ModReLU(nn.Module):
    ...
    def forward(self, x):
        logging.debug(f"Shape of x to ModReLU: {x.shape}")
        ...
        logging.debug(f"Shape of x out ModReLU: {x.shape}")
    ...
    def select_best(self, loss_fn, inputs, targets, mask, architecture):
        ...
        for model in self.population:
            ...
            if architecture == "Quaternion Transformer":
                ...
                logging.debug(f"Logits output QT: {logits.shape}")  
...
class QuaternionTransformerGUI:
    ...
    def create_widgets(self):
        ...
        ttk.Combobox(transformer_frame, textvariable=self.architecture, values=["Quaternion Transformer", "Quaternion Relational"], state="readonly").grid(row=0, column=3)
    ...
    def save_transformer_and_model(self):
        ...
        if directory:
            ...
            elif self.architecture.get() == "Quaternion Relational":
                model_file_name = 'quaternion_relational.pth'
            else:
    ...
    def load_tokenizer(self):
        try:
            ...
            if not hasattr(self.tokenizer, "model_max_length") or self.tokenizer.model_max_length > 1024 * 1024:
                self.tokenizer.model_max_length = seq_len  # Default to global seq_len for character-level tokens
    ...
    def load_model(self):
        try:
            ...
            if self.architecture.get() == "Quaternion Transformer":
                ...
                self.model = QuaternionTransformerRelational(
                    vocab_size=vocab_size,
                    embedding_dim=self.hidden_size.get(),
                    hidden_dim=self.hidden_size.get(),
                    num_layers=self.num_layers.get(),
            ...
            for epoch in range(self.epochs.get()):
                ...
                for batch_idx, (batch_input_ids, batch_labels, seq_lengths) in enumerate(dataloader):
                    ...
                    if self.use_genetic_algo == "Genetic Algorithm":
                        ...
                        self.model = neat.evolve(loss_fn, dataloader)
                        ...
                        self.model = firefly_optimizer.optimize(loss_fn, dataloader)
                    else:
                    # Forward pass
                        try:

                            if architecture == "Quaternion Transformer":
                                logits, _= self.model(batch_input_ids, combined_mask)
                                # Flatten logits and targets:
                                logging.debug(f"Logits after QT: {logits.shape}")  
                        ...
                        if torch.isnan(loss).any():
                            logging.error("NaN detected in loss computation!")
